...
ARCHIVE_EXPRESSION = (
    '__import__("_alembic_hom_extensions").alembicGetCameraDict'
)
...
def transfer_non_default_values(src, dest, ignore=None):
    ...
    for parm in src.allParms():
        ...
        if expression is not None and ARCHIVE_EXPRESSION in expression:
            # Assume it's part of the automated connections that the Alembic
            # Archive makes on loading of the camera and thus we do not want

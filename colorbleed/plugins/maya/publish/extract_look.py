...
def source_hash(filepath, *args):
    ...
    return "|".join([
                        os.path.normpath(os.path.abspath(filepath))
                        .lower().replace("\\", "/"),
                        str(os.path.getmtime(filepath)),
...
class ExtractLook(colorbleed.api.Extractor):
    ...
    def process(self, instance):
        ...
        for resource in resources:
            ...
            if source not in destinations:
                ...
                # multiple times
                destinations[source] = self.resource_destination(instance,
                                                                 source,
                                                                 do_maketx)
    ...
    def resource_destination(self, instance, filepath, do_maketx):
        ...
        return os.path.join(instance.data["assumedDestination"],
                            "resources",
                            basename + ext).replace(r"\\", "/")
